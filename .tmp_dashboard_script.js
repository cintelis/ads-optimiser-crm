...
async function renderSection(s) {
  ...
  c.innerHTML = '<div class="empty"><div class="empty-icon">â‹¯</div><p>Loading...</p></div>';
...
// â”€â”€ Loaders â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function renderOverview() {
  ...
  <div class="table-wrap">
    ...
    \${recent.length ? recent.map(l=>\`<tr><td class="mono" style="font-size:12px">\${l.contact_email}</td><td>\${l.subject||'â€”'}</td><td>\${l.campaign_name||'â€”'}</td><td><span class="badge badge-\${l.status}">\${l.status}</span></td><td class="text-muted text-sm">\${fmtDate(l.sent_at)}</td></tr>\`).join('') : '<tr><td colspan="5" style="text-align:center;color:var(--muted2);padding:32px">No emails sent yet.</td></tr>'}
    </tbody></table>
  </div>\`; 
}
...
function openTemplateModal(tmpl) {
  ...
  setModal(\`<div class="modal-head"><h3>\${t.id?'Edit Template':'New Template'}</h3><button class="modal-close" onclick="closeModal()">âœ•</button></div>
  <div class="modal-body">
    <div class="form-group"><label>Template Name</label><input id="t-name" value="\${esc(t.name)}" placeholder="e.g. AI Outreach â€” Intro"></div>
...
function renderContacts(q='') {
  ...
  <div class="toolbar">
    ...
    <button class="btn btn-ghost" onclick="openImportModal()">â¬† Import CSV</button>
  ...
  <div class="table-wrap">
    ...
    \${ct.length ? ct.map(c=>\`<tr>
      ...
      <td>\${esc(c.name)||'â€”'}</td>
      <td class="text-muted">\${esc(c.company)||'â€”'}</td>
...
function openContactModal(c) {
  ...
  setModal(\`<div class="modal-head"><h3>\${ct.id?'Edit Contact':'Add Contact'}</h3><button class="modal-close" onclick="closeModal()">âœ•</button></div>
  <div class="modal-body">
...
function openImportModal() {
  setModal(\`<div class="modal-head"><h3>Import Contacts (CSV)</h3><button class="modal-close" onclick="closeModal()">âœ•</button></div>
  <div class="modal-body">
    ...
    <div class="flex gap" style="margin-bottom:12px"><button class="btn btn-ghost btn-sm" onclick="document.getElementById('csv-file').click()">ðŸ“Ž Upload CSV file</button></div>
...
function renderLists() {
  ...
  <div class="table-wrap">
    ...
    \${ls.length ? ls.map(l=>\`<tr>
      ...
      <td class="text-muted">\${esc(l.description)||'â€”'}</td>
...
function openListModal() {
  setModal(\`<div class="modal-head"><h3>New Contact List</h3><button class="modal-close" onclick="closeModal()">âœ•</button></div>
  <div class="modal-body">
...
async function viewList(id, name) {
  ...
  setModal(\`<div class="modal-head"><h3>List: \${esc(name)}</h3><button class="modal-close" onclick="closeModal()">âœ•</button></div>
  <div class="modal-body">
    ...
    <div class="table-wrap">
      ...
      \${members.length ? members.map(m=>\`<tr><td class="mono" style="font-size:12px">\${esc(m.email)}</td><td>\${esc(m.name)||'â€”'}</td><td class="text-muted">\${esc(m.company)||'â€”'}</td><td><button class="btn btn-danger btn-sm" onclick="removeFromList('\${id}','\${m.id}','\${esc(name)}')">Remove</button></td></tr>\`).join('') : '<tr><td colspan="4" style="text-align:center;color:var(--muted2);padding:24px">No members yet.</td></tr>'}
      </tbody></table>
    </div>
  </div>\`);
}
...
function renderCampaigns() {
  ...
  <div class="table-wrap">
    ...
    \${cs.length ? cs.map(c=>\`<tr>
      ...
      <td class="text-muted">\${esc(c.list_name||'â€”')}</td>
      ...
      <td><div class="flex gap">
        ...
        <button class="btn btn-primary btn-sm" onclick="sendCampaignNow('\${c.id}','\${esc(c.name)}')">â–¶ Send</button>
...
function renderCampaignModal() {
  ...
  setModal(\`<div class="modal-head"><h3>New Campaign</h3><button class="modal-close" onclick="closeModal()">âœ•</button></div>
  <div class="modal-body">
    <div class="form-row">
      <div class="form-group"><label>Campaign Name *</label><input id="ca-name" placeholder="e.g. AI Outreach Q2"></div>
      <div class="form-group"><label>Contact List *</label><select id="ca-list"><option value="">â€” Select list â€”</option>\${lOpts}</select></div>
    </div>
    <div class="form-row">
    ...
    <div class="form-group"><label>Schedule Type *</label>
      <select id="ca-type" onchange="renderScheduleFields()">
        ...
        <option value="once">Send Once â€” at a specific time</option>
        <option value="recurring">Recurring â€” daily/weekly</option>
        <option value="drip">Drip Sequence â€” staggered multi-email</option>
      </select>
    </div>
  ...
  return campaignSteps.map((s,i)=>\`<div class="step-item">
    ...
    <div style="flex:1">
      <div class="form-row" style="margin-bottom:0">
        <div class="form-group" style="margin-bottom:0"><label>Template</label><select onchange="campaignSteps[\${i}].template_id=this.value"><option value="">â€” select â€”</option>\${tOpts.replace('value="'+s.template_id+'"','value="'+s.template_id+'" selected')}</select></div>
    ...
    \${campaignSteps.length>1?'<button class="btn btn-danger btn-sm" onclick="removeStep('+i+')">âœ•</button>':''}
  </div>\`).join('');
}
...
function renderLogs() {
  ...
  <div class="table-wrap">
    ...
    \${l.length ? l.map(r=>\`<tr>
      ...
      <td style="max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">\${esc(r.subject||'â€”')}</td>
      <td class="text-muted">\${esc(r.campaign_name||'â€”')}</td>
      <td class="text-muted">\${esc(r.template_name||'â€”')}</td>
      <td><span class="badge badge-\${r.status}">\${r.status}\${r.error?'<span title="'+esc(r.error)+'" style="cursor:help;margin-left:4px">â“˜</span>':''}</span></td>
...
// â”€â”€ Unsubscribes â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function renderUnsubs() {
  const u = state.unsubs;
  document.getElementById('content').innerHTML = \`
  <div class="toolbar">
    <div style="flex:1"><span class="text-muted text-sm">\${u.length} unsubscribed</span></div>
    <span class="text-muted text-sm" style="font-style:italic">Managed by the email worker Â· <a href="https://365soft-email-worker.nick-598.workers.dev/unsubscribe" target="_blank" style="color:var(--cyan)">Unsubscribe page â†—</a></span>
  </div>
  <div class="table-wrap">
    ...
    \${u.length ? u.map(c=>\`<tr>
      ...
      <td class="text-muted text-sm mono">\${esc(c.ip||'â€”')}</td>
      <td class="text-muted text-sm">\${esc(c.source||'â€”')}</td>
    </tr>\`).join('') : '<tr><td colspan="4" style="text-align:center;color:var(--muted2);padding:32px">No unsubscribes. ðŸŽ‰</td></tr>'}
    </tbody></table>
  </div>\`;
}
...
function fmtDate(s) { if(!s) return 'â€”'; try { return new Date(s).toLocaleString('en-AU',{day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'}); } catch { return s; } }
...
// â”€â”€ Pipeline Board â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    \${STAGE_ORDER.map(stage => {
      ...
      return \`<div class="pipeline-col stage-\${stage}">
        <div class="pipeline-head">
          ...
          <span class="pipeline-meta">\${cards.length}\${stageVal>0?' Â· '+fmtCurrency(stageVal):''}</span>
        </div>
        <div class="pipeline-cards">
          \${cards.map(c=>\`<div class="pipeline-card" onclick="openDrawer('\${c.id}')">
            ...
            \${c.follow_up_at&&isOverdue(c.follow_up_at)?\`<div style="font-size:10px;color:var(--pink);margin-top:5px">â° Follow-up overdue</div>\`:''}
          </div>\`).join('')}
        </div>
      </div>\`;
    }).join('')}
...
// â”€â”€ Follow-ups â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function renderFollowUps() {
  ...
  <div class="followup-card \${isOverdue(c.follow_up_at)?'followup-overdue':''}" onclick="openDrawer('\${c.id}')">
    ...
    <div style="flex:1;min-width:0">
      ...
      <div class="text-muted text-sm">\${esc(c.company||'')} Â· <span class="badge badge-draft">\${STAGE_LABELS[c.stage]||c.stage}</span></div>
    </div>
  ...
  </div>\`).join('') : \`<div class="empty"><div class="empty-icon">âœ“</div><p>No follow-ups due â€” you're all caught up!</p></div>\`}
  \`;
}

// â”€â”€ Contact Drawer â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function openDrawer(contactId) {
  ...
  document.getElementById('drawer-body').innerHTML = \`<div class="empty"><div class="empty-icon">â‹¯</div><p>Loading...</p></div>\`;
...
function renderDrawerContent(d) {
  ...
  <div class="drawer-section">
    ...
    <div class="info-grid">
      ...
      <div class="info-item"><label>Phone</label><span>\${esc(c.phone||'â€”')}</span></div>
      <div class="info-item"><label>Company</label><span>\${esc(c.company||'â€”')}</span></div>
      <div class="info-item"><label>LinkedIn</label>\${c.linkedin?\`<a href="\${esc(c.linkedin)}" target="_blank" style="color:var(--cyan);font-size:12px">View Profile â†—</a>\`:'<span>â€”</span>'}</div>
  ...
  <div class="drawer-section">
    ...
    <div style="display:flex;gap:8px;margin-bottom:12px">
      <select id="note-type" style="background:#0d0d15;border:1px solid var(--border2);border-radius:6px;padding:6px 10px;color:var(--text);font-size:12px">
        <option value="note">ðŸ“ Note</option><option value="call">ðŸ“ž Call</option><option value="meeting">ðŸ¤ Meeting</option><option value="email">âœ‰ Email</option>
      </select>
    ...
    \${d.notes.length ? d.notes.map(n=>\`<div class="note-item" id="note-\${n.id}">
      ...
      <div class="flex" style="justify-content:space-between;align-items:center;margin-top:4px">
        ...
        <button class="btn btn-danger btn-sm" style="padding:2px 8px" onclick="deleteNoteFromDrawer('\${c.id}','\${n.id}')">âœ•</button>
      </div>
  ...
  <div class="drawer-section">
    ...
    \${d.emails.length ? \`<div class="table-wrap"><table>
      ...
      <tbody>\${d.emails.map(e=>\`<tr>
        <td style="font-size:12px">\${esc(e.subject||'â€”')}</td>
...
function openContactEditModal(id) {
  ...
  setModal(\`<div class="modal-head"><h3>Edit Contact</h3><button class="modal-close" onclick="closeModal()">âœ•</button></div>
  <div class="modal-body">
    <div class="form-row">
...
function editTags(id, tags) {
  setModal(\`<div class="modal-head"><h3>Edit Tags</h3><button class="modal-close" onclick="closeModal()">âœ•</button></div>
  <div class="modal-body">
